...
CARTIM = [[170, 446], [196, 380]]
CARTBOT = [[-0.3,0.3], [-0.4,-0.8]]
GOAL = [600,300]
ZLOW = -0.065#-0.085166662
ZHIGH = 0.259638717
...
def detectBlock():
    ...
    und_im = cv2.undistort(im, CAMERAMATRIX, DISTORTION)
    ...
    cv2.drawContours(und_im, conts, -1, (255, 255, 0), 1)
        
    centers = [getCenter(*cv2.boundingRect(c)) for c in conts]
    return [pixelsToCartesian(*c) for c in centers]
